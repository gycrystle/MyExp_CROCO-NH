...
# elif GLS_VADV_UP3
!
! Compute vertical advective fluxes using 3th-order upwind scheme
! (1st near top and bottom if there is not enough space to lay the
! three points stencil)
!
          do k=1,N-1
            do i=Istr,Iend
               if (We(i,j,k).gt.0.)then
                  ! upward flux
                  if(k.gt.1)then
                     FC(i,k)=We(i,j,k)*(-tmp(i,j,k-1,3)+5*tmp(i,j,k,3)
     &                    +2*tmp(i,j,k+1,3))*0.166666666666
                  else
                     FC(i,k)=We(i,j,k)*tmp(i,j,k,3)
                  endif
               else
                  ! downward flux
                  if(k.lt.N-1)then
                     FC(i,k)=We(i,j,k)*(2*tmp(i,j,k,3)+5*tmp(i,j,k+1,3)
     &                    -tmp(i,j,k+2,3))*0.166666666666
                  else
                     FC(i,k)=We(i,j,k)*tmp(i,j,k+1,3)
                  endif
               endif
            enddo
          enddo
